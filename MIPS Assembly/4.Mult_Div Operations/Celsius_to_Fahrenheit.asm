...
cTemp: .word 25        # Celsius temperature variable initialized to 25
...
lw $s1, 0($s0)         # Load contents at address stored in $s0 into $s1 (cTemp = 25)
...
mflo $s2               # Move lower 32 bits of result of multiplication into $s2 (s2 = 25 * 9 = 225)
li $t1, 5              # Load constant 5 into $t1 (for division)
div $s2, $t1           # Divide result in $s2 by 5 and store quotient in $s3
mflo $s3               # Move result of division into $s3 (s3 = 225 / 5 = 45)
addi $s3, $s3, 32      # Add 32 to quotient to convert Celsius to Fahrenheit (s3 = 45 + 32 = 77)
